...
export default function App() {
  ...
  const getUrlState = () => {
    ...
    return {
      view: tab,
      selectedDoctor: tab === 'doctor-details' ? getSelectedItem('doctor') : null,
      selectedHospital: tab === 'hospital-details' ? getSelectedItem('hospital') : null,
      selectedAppointment: tab === 'appointment-details' ? getSelectedItem('appointment') : null,
    };
  };
  ...
  const navigateTo = (newView, data = {}) => {
    ...
    // Doctor Navigation
    if (data.doctor) {
      params.set('doctorId', data.doctor.user_id || data.doctor.id);
      setSelectedDoctor(data.doctor);
      setSelectedItem('doctor', data.doctor);
    } else if (newView === 'doctors' || newView === 'dashboard') {
      setSelectedDoctor(null);
    ...
    // Hospital Navigation
    if (data.hospital) {
      params.set('hospitalId', data.hospital.user_id || data.hospital.id);
      setSelectedHospital(data.hospital);
      setSelectedItem('hospital', data.hospital);
    } else if (newView === 'hospitals' || newView === 'dashboard') {
      setSelectedHospital(null);
    ...
    if (data.appointment) {
      ...
      setSelectedAppointment(data.appointment);
      setSelectedItem('appointment', data.appointment);
    } else if (newView === 'appointments' || newView === 'dashboard') {
      setSelectedAppointment(null);
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white border border-slate-200 w-full max-w-lg rounded-2xl shadow-xl p-8 text-center space-y-6">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex items-center gap-3 text-left">
            <LifeBuoy className="w-5 h-5 text-slate-400 flex-shrink-0" />
            <span className="text-xs text-slate-500 font-medium">
              To appeal this status or request profile recovery, contact helpdesk at <strong className="text-slate-700">support@docapp.co.in</strong>.
            </span>
          </div>
          <button 
            onClick={() => {
  ...
  const renderActiveView = () => {
    switch (view) {
      case 'dashboard':
        return <Dashboard setView={(tab) => navigateTo(tab)} />;
      case 'doctors':
        return <DoctorSearch onSelectDoctor={(doc) => navigateTo('doctor-details', { doctor: doc })} />;
      case 'doctor-details':
        return (
          <DoctorDetails
            doctor={selectedDoctor}
      ...
      case 'hospitals':
        return <HospitalSearch onSelectHospital={(hospital) => navigateTo('hospital-details', { hospital })} />;
      case 'hospital-details':
        return (
          <HospitalDetails
            hospital={selectedHospital}
      ...
      case 'appointments':
        return <AppointmentList onSelectAppointment={(app) => navigateTo('appointment-details', { appointment: app })} />;
      case 'appointment-details':
        return (
          <AppointmentDetails
            appointment={selectedAppointment}
