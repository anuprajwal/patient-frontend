...
export default function AppointmentDetails({ appointment, onBack }) {
  ...
  // Helper to handle double-escaped JSON strings from the backend
  const parseSlotsData = (slotsField) => {
    if (!slotsField) return [];
    let parsed = slotsField;
  ...
  const syncAppointmentExtraData = async () => {
    ...
    try {
      ...
      if (doctorId && patientEndpoints.getDoctorSlots) {
        ...
        setAvailableSlots(parseSlotsData(rawSlots));
      }
    } catch (err) {
      /* Silent handling for missing sub-resources */
    } finally {
