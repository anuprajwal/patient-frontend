...
// export default function AppointmentDetails({ appointment: propAppointment, onBack }) {
//   const [currentAppointment, setCurrentAppointment] = useState(() => propAppointment || getPersistedSelection('appointment'));
...
//   useEffect(() => {
//     if (propAppointment) {
//       setCurrentAppointment(propAppointment);
//       persistSelection('appointment', propAppointment);
//     }
//   }, [propAppointment]);

//   const doctor = currentAppointment?.doctor || {};
//   const profile = doctor.doctorProfile || {};
//   const statusMeta = getAppointmentStatusMeta(currentAppointment?.appointment_status);
//   const appType = (currentAppointment?.appointment_type || '').toLowerCase();
//   const isAppointmentClosed = (currentAppointment?.appointment_status || '').toLowerCase() === 'closed';
//   const checkupAppointments = Array.isArray(currentAppointment?.checkupAppointment) ? currentAppointment.checkupAppointment : [];
...
//     setError('');
//     try {
//       const docRes = await patientEndpoints.getDocumentsForAppointment(currentAppointment.id);
//       setDocuments(docRes.data?.documents || docRes.data || []);

//       const presRes = await patientEndpoints.getPrescriptionForAppointment(currentAppointment.id);
...
//         <p className="text-slate-500 mb-4 font-medium">No appointment selected or session expired.</p>
...
//     try {
//       const targetDoctorId = doctor.user_id || doctor.id || currentAppointment.doctor_id;
...
//       const parentDate = new Date(currentAppointment.appointment_date);
...
//     const payload = {
//       appointment_id: String(currentAppointment.id),
//       date: targetDay.date,
...
//     const doctorUserId = doctor.user_id || doctor.id || currentAppointment.doctor_id;
...
//     fd.append('appointment_id', String(currentAppointment.id));
...
//   const formattedDate = currentAppointment?.appointment_date ? new Date(currentAppointment.appointment_date).toLocaleDateString('en-US', {
...
//               <div>
//                 <span className="text-xs font-extrabold text-slate-400 uppercase tracking-wider">Booking #{currentAppointment.id}</span>
...
//                   <Clock className="w-3.5 h-3.5 text-slate-400" /> {currentAppointment.appointment_start_time} - {currentAppointment.appointment_end_time}
//                 </p>
//               </div>
...
//                   {appType === 'online_video' ? <Video className="w-4 h-4 text-blue-500" /> : <MapPin className="w-4 h-4 text-rose-500" />}
//                   {currentAppointment.appointment_type?.replace('_', ' ')}
//                 </p>
//                 <p className="text-slate-600 font-semibold capitalize mt-1">
//                   Payment Mode: <strong className="text-slate-800">{currentAppointment.payment_mode}</strong>
//                 </p>
//               </div>
//             </div>
...
//               rawPrescriptionString={currentAppointment.prescription}
//               documents={documents}
//               loading={loading}
...
//                 Showing available slots for Dr. {doctor.username || 'Doctor'} starting 5 days after appointment #{currentAppointment.id}.
//               </p>
//             </div>
...
export default function AppointmentDetails({ appointment: propAppointment, onBack }) {
  // 1. Read from prop or retrieve the single active stored appointment on reload
  const appointment = propAppointment || getSelectedItem('appointment');
  ...
  // 2. Fetch fresh documents & prescription from the API on mount
  const syncAppointmentExtraData = async () => {
  ...
  // 3. Clear the single stored item on clicking back
  const handleBack = () => {
    clearSelectedItem('appointment');
    onBack();
  };
  ...
  const handleExecuteCheckupSchedule = async () => {
    ...
    try {
      ...
      const { orderId, amount: totalAmount, key } = resData;
      if (!window.Razorpay) throw new Error('Razorpay SDK script is unavailable.');
