...
import { ArrowLeft, Calendar, Clock, Video, MapPin, X, CheckCircle2, CalendarPlus, Star, MessageSquare } from 'lucide-react';
...
export default function AppointmentDetails({ appointment: propAppointment, onBack }) {
  ...
  // Checkup Scheduling Modal State
  ...
  const currentStatus = (appointment?.appointment_status || '').toLowerCase();
  const isAppointmentClosed = currentStatus === 'closed' || currentStatus === 'completed';
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      await patientEndpoints.submitDoctorReview({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {/* Review Trigger Button for Completed Consultations */}
        {isAppointmentClosed && (
          <button
            onClick={() => setShowReviewModal(true)}
            type="button"
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-amber-500 hover:bg-amber-600 text-white transition shadow-sm"
          >
            <Star className="w-3.5 h-3.5 fill-current" /> Rate & Review Doctor
          </button>
        )}
      </div>
      ...
      {showReviewModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 w-full max-w-lg rounded-2xl shadow-2xl p-6 space-y-4 relative animate-fadeIn">
            <button 
              onClick={() => { setShowReviewModal(false); setReviewError(''); setReviewSuccess(''); }}
            ...
            <div className="flex items-center gap-2 text-slate-900 font-bold text-lg">
              ...
              <span>Review Dr. {doctor.username || 'Doctor'}</span>
            </div>
            
            <p className="text-xs text-slate-500">
              Share your feedback regarding your consultation on {formattedDate}.
            </p>
            ...
            <form onSubmit={handleSubmitReview} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe your consultation experience, doctor punctuality, diagnosis clarity..."
                  className="w-full p-3 border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500 bg-slate-50/50 outline-none resize-none"
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submittingReview}
                  className="px-5 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow-sm transition"
                >
