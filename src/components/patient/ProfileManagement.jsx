...
export default function ProfileManagement() {
  ...
  const syncProfileData = async () => {
    ...
    try {
      const userRes = await patientEndpoints.getUserData();
      // Destructure directly based on the incoming { success: true, userData: {...} } structure
      const rawUserData = userRes.data?.userData || userRes.data;
      setUserData(rawUserData);
      
      if (rawUserData) {
        // Map the fields out of the nested generalUser object safely
        setProfileForm({
  ...
  // Safely grab the profile picture from the nested object layer
