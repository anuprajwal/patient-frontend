...
//       setError('');
//       try {
//         // RECTIFIED: Explicitly target user_id first (e.g., 4) instead of database record table id (e.g., 2)
//         const targetId = doctor.user_id || doctor.id;
        
//         // 1. Fetch Schedule Slots using user_id parameter signature mapping
...
//         // 2. Fetch Live Dynamic Rating Data Matrix using user_id parameter signature mapping
...
//       } catch (err) {
//         // Safe structural interface local fallback
//         setReviews([
...
//       const payload = {
//         // RECTIFIED: Secure payload tracking parameters utilize doctor.user_id context explicitly
...
export default function DoctorDetails({ doctor, onBack }) {
  ...
  useEffect(() => {
    const syncDoctorDetailsData = async () => {
      ...
      try {
        ...
        // 1. Fetch Doctor Schedule Slots using user_id
        const slotsResponse = await patientEndpoints.showDoctorSlots(targetId);
        let baseSlotsElement = slotsResponse.data?.slots?.[0]?.slots || slotsResponse.data?.slots || doctor.user?.doctorSlots?.slots;
  ...
  const executeAppointmentBooking = async () => {
    ...
    const doctorUserId = String(doctor.user_id || doctor.id);
    ...
    try {
      ...
      // ------------------------------------------------------------------
      if (paymentMode === 'cash') {
        await patientEndpoints.confirmAppointment({ appointmentId: String(createdAppointmentId) });
        setBookingSuccess(`Appointment #${createdAppointmentId} successfully created and confirmed for Cash Desk Payment!`);
        resetSelection();
        setSubmittingBooking(false);
        return;
      }
      ...
      // ------------------------------------------------------------------
      // PATH B: ONLINE / CARD PAYMENT (Razorpay Gateway Flow)
      // ------------------------------------------------------------------
      if (paymentMode === 'card') {
        const amount = doctor.consultation_fee || 500;
        
        // 2. Create Razorpay Order via Backend Payment Controller
        ...
        const options = {
          ...
          description: `Consultation with ${doctor.user?.username || 'Doctor'}`,
          order_id: orderId,
          handler: async function (razorpayResponse) {
            try {
              ...
              if (verifyRes.data?.success) {
                // 4. Confirm Appointment State
                await patientEndpoints.confirmAppointment({ appointmentId: String(createdAppointmentId) });
                setBookingSuccess(`Payment verified! Appointment #${createdAppointmentId} is fully confirmed.`);
                resetSelection();
              } else {
          ...
          modal: {
            ondismiss: function () {
              setSubmittingBooking(false);
              setError("Payment window closed. Appointment generated in pending state.");
            }
          }
        };
      ...
      setError(err.response?.data?.message || err.message || 'Appointment booking process failed.');
      setSubmittingBooking(false);
    }
  };
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4 mt-4">
                  ...
                  <div className="flex justify-end pt-2">
                    <button
                      ...
                      className="bg-brand-600 hover:bg-brand-700 disabled:opacity-40 text-white text-xs font-bold px-5 py-2.5 rounded-lg transition-colors shadow-sm"
                    >
                      {submittingBooking ? 'Processing Payment & Booking...' : 'Confirm Ledger Booking'}
                    </button>
                  </div>
                </div>
              )}
